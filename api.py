...
"""
    ...
    def write_outputs(recipient_label, include_action, timestamp, timeline):
        ...
        report_role = role_names.get(recipient_label, recipient_label.title())

        display_time = datetime.now().strftime("%d %b %Y at %H:%M")

        full_text = f"""AIVS REPORT – {report_role} – {display_time}
...
"""
        ...
        full_text += "\n\n—\nThis report was generated by AIVS Software Limited. It uses classic strategic context and AI to enhance decision support, but it is not legal or financial advice."
    ...
    files_to_send = {
        email: write_outputs("user", include_action=False, timestamp=timestamp, timeline=timeline)
    }

    if supervisor_email:
        files_to_send[supervisor_email] = write_outputs("supervisor", include_action=True, timestamp=timestamp, timeline=timeline)

    if hr_email:
        files_to_send[hr_email] = write_outputs("hr", include_action=True, timestamp=timestamp, timeline=timeline)
